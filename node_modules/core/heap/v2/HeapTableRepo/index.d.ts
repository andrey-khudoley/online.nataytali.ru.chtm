...
import {
  ...
  HqExprColumn,
  HqNonAggregateColumnName,
  HqOutputSchemaForColNames,
  HqOutputSchemaProps,
  HqOutputTuple,
  HqOutputTupleForColNames,
  HqSelectClause,
  HqTupleForSchema,
} from 'core/heap/hqb/HeapSelectQueryBuilder'
import * as hs from 'core/heap/v2/heapSchemaTypes'
import { HeapTableRecordApi, OptionalHeapTableInfo } from 'core/heap/v2/HeapTableRecordApi'
import { Icon } from 'lib/chatium-json/v1'
import { DesktopLayout, PlainIconName } from 'lib/chatium-json/v2'
import { AudioFileClass, FileClass, ImageFileClass, VideoFileClass } from 'modules/storage/StorageFile'
...
/**
 * Generic read-only repository for the heap-stored objects.
 * New JSON-schema-based heap-table edition.
 * This abstract class contains shared code between backend core and UGC implementations.
 */
export declare abstract class ReadOnlyHeapTableRepo<HSP extends hs.HsProperties, ReadCtx, WriteCtx = ReadCtx> {
  readonly type: HeapObjectType
  ...
  findAll(ctx: ReadCtx, options?: HsTableSelectOptions<HSP>): Promise<this['T'][]>
  findAllIds(ctx: ReadCtx, options?: HsTableSelectOptions<HSP>): Promise<HeapId[]>
  findBy(ctx: ReadCtx, filter?: HsTableFilter<HSP> | null): Promise<this['T'][]>
  findOneBy(ctx: ReadCtx, filter?: HsTableFilter<HSP> | null): Promise<this['T'] | null>
  countBy(ctx: ReadCtx, filter?: HsTableFilter<HSP> | null): Promise<number>
  select<SE extends HqSelectClause<HsFullProps<HSP>>>(
    selections: Deny$StartingKeys<SE>,
  ): HeapSelectQueryBuilder<
    HsFullProps<HSP>,
    HqOutputSchemaProps<HsFullProps<HSP>, SE>,
    HqOutputTuple<HsFullProps<HSP>, SE>,
    HqNonAggregateColumnName<HsFullProps<HSP>, SE>,
    ReadCtx
  >
  select<ColNames extends Array<HqExprColumn<HsFullProps<HSP>, hs.HeapSchema>>>(
    ...colNames: ColNames
  ): HeapSelectQueryBuilder<
    HsFullProps<HSP>,
    HqOutputSchemaForColNames<HsFullProps<HSP>, ColNames>,
    HqOutputTupleForColNames<HsFullProps<HSP>, ColNames>,
    ColNames[number],
    ReadCtx
  >
  selectAll(): HeapSelectQueryBuilder<
    HsFullProps<HSP>,
    HsFullProps<HSP>,
    HqTupleForSchema<HsFullProps<HSP>>,
    Array<keyof HsFullProps<HSP>>,
    ReadCtx
  >
  searchBy(ctx: ReadCtx & OptionalEnvCtx, options: HsTableSearchByOptions<HSP>): Promise<Array<this['T']>>
  /**
  ...
  private get selectAllClause()
}
export declare const systemColumnsSelectClause: {
  id: string
  createdAt: string
  updatedAt: string
  createdBy: string
...
declare type HsFullProps<HSP extends hs.HsProperties> = HSP & hs.HeapObjectFilterableSchema
/**
 * Generic repository for the heap-stored objects.
 * Extends read-only repository with write operations.
 */
export declare abstract class HeapTableRepo<
  HSP extends hs.HsProperties,
  ReadCtx,
  WriteCtx,
> extends ReadOnlyHeapTableRepo<HSP, ReadCtx, WriteCtx> {
  /**
  ...
  updateAll(ctx: WriteCtx, options: HsUpdateAllOptions<HSP>): Promise<number>
  ...
  deleteAll(ctx: WriteCtx, options: HsDeleteAllOptions<HSP>): Promise<number>
  /**
   ...
   * It's necessary only in one case - auto-migration of RefLinks from old getcourse user cache to v2 (Multi) users.
   * @internal
...
export interface HeapRuntimeClasses {
...
export interface HeapTableOptions<HSP extends hs.HsProperties> extends hs.HsCustomMetaOption {
  getRecordTitleSync?(record: hs.HsFullObjectRuntime<HSP>): string | null | undefined
}
declare type CreateOrUpdateUniqValue<HSP extends hs.HsProperties, K extends keyof HSP> = tb.Static<HSP[K]> extends
  | string
  | number
